...
const extension: ExtensionFactory = (pi) => {
  pi.registerCommand("deploy", {
    ...
    handler: async (_args, ctx) => {
      ...
      // Check if scene has worldConfiguration to auto-detect target
      let isWorldDeploy = false;
      try {
        let content = await readFile(join(sceneRoot, "scene.json"), "utf-8");
        if (content.charCodeAt(0) === 0xfeff) content = content.slice(1);
        const sceneJson = JSON.parse(content);
        if (sceneJson.worldConfiguration?.name) {
          isWorldDeploy = true;
        }
      } catch {
        // If we can't read scene.json, proceed with default deploy
      }
      ...
      try {
        ...
        if (result.exitCode === 0) {
          ...
          ctx.ui.notify(`Deploy failed (exit code ${result.exitCode}): ${result.stderr || result.stdout}`, "error");
        }
      } catch (err) {
